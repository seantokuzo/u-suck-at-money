...
export function RetirementPlanForm({
  ...
  const handleSubmit = (formData: FormData) => {
    startTransition(async () => {
      if (isEdit) {
        ...
        await createRetirementPlan(formData);
      }
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={isEdit ? "Edit 401k Plan" : "Add 401k Plan"}
    >
      <form
        ref={formRef}
        action={handleSubmit}
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input
            ...
            max="100"
            placeholder="e.g. 50"
            defaultValue={plan?.employerMatchPct?.toString() ?? ""}
          />
          ...
          <Input
            ...
            defaultValue={centsToDisplay(plan?.employerMatchCap)}
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input
            ...
            inputMode="decimal"
            placeholder="0.00"
            defaultValue={centsToDisplay(plan?.vestedBalanceCents)}
          />
          ...
          <Input
            ...
            inputMode="decimal"
            placeholder="0.00"
            defaultValue={centsToDisplay(plan?.totalBalanceCents)}
          />
        </div>
        ...
        <div className="mt-2 flex items-center justify-end gap-3">
          ...
          <Button type="submit" loading={isPending}>
            {isEdit ? "Save Changes" : "Add 401k Plan"}
          </Button>
        </div>
      </form>
    </Modal>
